...
class WC_Woo_Product_Parceled {
	...
	private static $new_version = '1.5.9';	
	/*
	 * Inicia tudo
	...
	public static function woo_product_parceled_installment($parceled, $get_price) {	
		...
		if($pagseguro_settings_installment > 0) {		
	...
	public function woo_product_parceled_loop() {
		...
		$product = get_product();
		$pagseguro_settings = get_option( 'woocommerce_pagseguro_settings' );
		if ($pagseguro_settings[installment] > 0) { ?>	
		...
		<div class="p-woo-pagseguro-price">
			...
			<?php printf(__('Ou em até <span>%sx</span> de <span>%s</span>', 'woo-pagseguro-parceled' ), $this->woo_product_parceled_installment($pagseguro_settings[installment], $product->get_price_including_tax()), $this->woo_product_parceled() ); ?><br/>
	...
	public function woo_product_parceled_single_product() {
		...
		if ($pagseguro_settings[installment_single_product] == "yes") { ?>
